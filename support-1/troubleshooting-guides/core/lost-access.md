...
Use the `defguard manage` commands when the Web UI or API become unavailable because of user or group problems. This tool connects directly to the database, you need to run it with the same [database settings](lost-access.md#setting-database-credentials) as the running Defguard server.
...
If the only admin forgot their password or lost access, reset it from the CLI.

```bash
defguard manage change-password --username <username> --password <new-password>
```

Log in with the new password. If the account also lost its admin group, follow the next section.
...
### Stop directory sync from overwriting groups
