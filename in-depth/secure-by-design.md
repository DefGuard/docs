...
Defguard separates all components and enables **deployment of the main component** (Core) which is responsible for business logic and has access to user data (e.g., via integration with SSO, LDAP, or Active Directory) **in a fully isolated environment**. The Core can be deployed without direct Internet exposure and is not accessible from application components that are publicly exposed (e.g., the Edge component).
...
### Inspection, verification and security audits
