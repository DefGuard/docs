...
2. Navigate to **APIs & Services → OAuth consent screen** and configure the consent screen for your organization.
...
* **Encryption mismatch**: make sure the port and the encryption mode are consistent — for example, port `465` requires _Implicit TLS_, while port `587` is used with _StartTLS_. Mixing these will cause the connection to fail or produce a TLS handshake error.
* **Credentials**: for the _Username and password_ option, double-check the username and password. If your provider requires an app-specific password, make sure you are using that instead of your regular account password.
* **OAuth2 token**: for the _Google_ and _Microsoft_ options, the OAuth2 token may have expired or been revoked. Re-open the configuration panel and go through the authentication flow again to obtain a fresh token.
