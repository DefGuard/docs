...
* True Zero-Trust [WireGuard® VPN with 2FA/Multi-Factor Authentication](../features/wireguard/),
* Identity Management with [SSO based on OpenID Identity Provider](../features/openid-connect/),
* Account Lifecycle management with [secure remote account onboarding](../using-defguard-for-end-users/enrollment/).
...
Defguard is a true Zero-Trust [WireGuard® VPN with 2FA/Multi-Factor Authentication](../features/wireguard/), as each connection requires MFA (and not only when logging in into the client application like other solutions):
...
1. Internal IdP with 2FA/MFA enables us to provide [**real VPN 2FA/MFA**](../in-depth/architecture/architecture.md) - and not like most applications just 2FA when opening the app (and not during the connection process). Even if you use [external OIDC](../features/external-openid-providers/) (Google/Microsoft/Custom - which Defguard supports), we still use our internal IdP for 2FA/MFA.
